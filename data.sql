...
item_name='Veg Wrap',item_price='55',item_availability='Yes',item_launch_date=GETDATE(),item_category='Starters',item_delivery='Yes'
where item_id=4
go
...
from menu_items m inner join cart c
...
select sum(m.item_price) as Total_price,u.user_id
from menu_items m inner join cart c
on c.item_id=m.item_id 
inner join user u on 
u.user_id=c.user_id
...
delete cart where user_id=3 and item_id=1
go
